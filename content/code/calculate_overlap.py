...
def calculate_overlap(tile_data,num_proc=20):
    ...
    overlaps = pool.map(partial(determine_overlap), row_pairs)
    pool.close()
    pool.join()
...
def determine_overlap(doubled):
    ...
    with mrcfile.open(im1["tile_mask_filename"]) as mrc: 
        ...
        area1 = np.sum(reference_mask > 0.9)
...
output_directory = Path("/scratch/bern/elferich/deco_lace_manuscript_processing/refined_assembly_overlap/")
...
for (database, name) in utils.dataset_info[:]:
    ...
    overlaps = calculate_overlap(tile_data,num_proc=20)
    ratio = []
    for overlap in overlaps:
        ratio.append(overlap[2]*2/(overlap[1]+overlap[0]))
    print(np.mean(ratio))
