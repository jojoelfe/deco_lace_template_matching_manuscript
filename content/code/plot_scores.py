...
with lp.figure("scores_plot",tight_layout=False):
    ...
    axs[0].set_title(f"Eucentric Focus \n median = {np.median(np.concatenate(scores[0:4])):.1f}")
    axs[1].set_title(f"Fringe-free Focus \n median = {np.median(np.concatenate(scores[4:8])):.1f}")
    ...
    pos = np.arange(4) + 1
    upper_labels = [f"{np.median(s):.1f}" for s in scores[0:4]]
    ...
    pos = np.arange(4) + 1
    upper_labels = [f"{np.median(s):.1f}" for s in scores[4:8]]
    weights = ['bold', 'semibold']
