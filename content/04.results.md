...
by an autophagocytic pathway. These granules were
...
described earlier (Fig. [@fig:matching_euc] F, Fig. [@fig:matching_fff]
...
[@fig:matching_stat] A). Lamella~EUC~ 1 has in the most targets, but also has
...
scores between lamellae, we found them to be fairly comparable with a medians
...
for DeCo-LACE (Figure [@fig:matching_stat]D).
...
"back" edge of the lamella (Fig. [@fig:matching_stat] E]). We reasoned that
this might be the case, because the center of the cell is predominantly
occupied by the nucleus, despite its segmentation in neutrophil-like cells. The
increase in matches at the "back" of the lamella compared to the "front" can also
be explained by the thickness gradient of the lamella (Fig. [@fig:lamella_motion_thickness] B,
Fig. [@fig:matching_euc]B, Fig. [@fig:matching_fff] B). In addition, we expected
that aberration would cause average 2DTM SNRs to be higher when beam-image shift
values are small. Instead, we found that SNRs where on average the highes at the
"front" edge of the lamella, presumably due to the thinner sample. We therefore
conclude that at present factors other that beam-image shift or beam
